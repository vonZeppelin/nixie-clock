 ...
 * if (client.requestTime("time.foo.com", 1000)) {
 *     long now = client.getNtpTime() + TimeUnit.NANOSECONDS.toMillis(System.nanoTime()) - client.getNtpTimeReference();
 * }
 * </pre>
 */
public class SntpClient {
    ...
    // value of NANOSECONDS.toMillis(System.nanoTime()) corresponding to mNtpTime
    ...
    public boolean requestTime(String host, int timeout) {
        try (DatagramSocket socket = new DatagramSocket()) {
            ...
            long requestTicks = NANOSECONDS.toMillis(System.nanoTime());
            writeTimeStamp(buffer, TRANSMIT_TIME_OFFSET, requestTime);

            socket.send(new DatagramPacket(buffer, buffer.length, InetAddress.getByName(host), NTP_PORT));

            // read the response
            socket.receive(new DatagramPacket(buffer, buffer.length));
            long responseTicks = NANOSECONDS.toMillis(System.nanoTime());
    ...
    /**
     * Returns the reference clock value (value of NANOSECONDS.toMillis(System.nanoTime()))
